/**
 * AgentWatch — popup.js (v1.5)
 * FIX: renderHistory card-actions div missing <, and escaped \${} template literals fixed.
 */
...
function initContentDelegation() {
  ...
  root.addEventListener('click', async (e) => {
    ...
    if (btn?.id === 'reply-send') {
      ...
      if (isReplyMode) {
        try { chrome.windows.getCurrent(w => chrome.windows.remove(w.id)); } catch {}
        return;
      }
      await renderTab(currentTab);
      return;
    ...
    if (btn?.classList.contains('details-btn')) {
      ...
      if (svg) svg.setAttribute('points', isOpen ? '6 9 12 15 18 9' : '18 15 12 9 6 15');
      return;
    }

    if (btn?.classList.contains('show-btn')) {
      e.stopPropagation();
      await jumpToTab(parseInt(btn.dataset.tabId), parseInt(btn.dataset.windowId), btn.dataset.url);
      return;
    }
    ...
    if (btn?.classList.contains('stop-monitoring-btn')) {
      ...
      try {
        await chrome.runtime.sendMessage({ type: 'STOP_CLI_SESSION', sessionId, siteName });
      } catch {}
    ...
    if (!btn) {
      ...
      if (!card) return;
      await jumpToTab(parseInt(card.dataset.tabId), parseInt(card.dataset.windowId), card.dataset.url);
    }
  });
...
async function renderMonitor(el) {
  ...
  const replyAge = pendingReply ? Date.now() - new Date(pendingReply.timestamp).getTime() : Infinity;
  ...
  if (pendingReply && replyAge < 30 * 60 * 1000) {
    html += `
      <div class="reply-panel" data-testid="reply-panel">
        ...
        ${pendingReply.messageSnippet ? `<div class="reply-snippet" data-testid="reply-snippet">${esc(pendingReply.messageSnippet)}</div>` : ''}
        <textarea id="reply-input" class="reply-input" data-testid="reply-input" placeholder="Type your reply here…" rows="3"></textarea>
        <div class="reply-actions">
          ...
          <button class="btn btn-primary" id="reply-send"
            ...
            data-testid="reply-send">Send Reply</button>
        </div>
  ...
  if (disabled) {
    html += `<div class="disabled-notice" data-testid="disabled-notice">
  ...
  if (sessions.length === 0) {
    html += `<div class="empty-state" data-testid="monitor-empty">
    ...
    for (const s of sessions) {
      html += `
        <div class="session-card" data-tab-id="${s.tabId}" data-window-id="${s.windowId || ''}" data-url="${esc(s.url || '')}" data-testid="session-card-${esc(s.site)}">
          <div class="session-row">
            <div class="session-site"><div class="pulse-dot"></div>${esc(s.siteName)}</div>
            <div class="session-timer" data-start="${s.startTime}" data-testid="session-timer-${esc(s.site)}">${fmtTimer(Date.now() - s.startTime)}</div>
          </div>
          <div class="session-title">${esc(shortenTitle(s.title || s.url))}</div>
          <div class="card-actions">
            <button class="btn btn-secondary btn-sm show-btn"
              data-tab-id="${s.tabId}" data-window-id="${s.windowId || ''}" data-url="${esc(s.url || '')}"
            ...
            <button class="btn btn-ghost btn-sm reply-btn"
              data-tab-id="${s.tabId}" data-window-id="${s.windowId || ''}"
              data-site="${esc(s.site)}" data-site-name="${esc(s.siteName)}" data-event-type="COMPLETED" data-snippet=""
...
async function renderHistory(el) {
  ...
  if (history.length === 0) {
    el.innerHTML = `<div class="empty-state" data-testid="history-empty">
      <div class="empty-icon">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#6b7592" stroke-width="1.5" stroke-linecap="round">
  ...
  for (const ev of history.slice(0, 30)) {
    ...
    if (level !== 'never') {
      ...
      replyBtnHtml = `<button class="btn ${btnClass} btn-sm reply-btn"
        data-tab-id="${btnTabId}" data-window-id="${btnWindowId}"
        data-site="${esc(ev.site)}" data-site-name="${esc(ev.siteName || ev.site)}"
        data-event-type="${esc(ev.eventType)}" data-category="${esc(ev.category || '')}"
    ...
    let stopBtnHtml = '';
    if (ev.isCLI || ev.site === 'cli') {
      stopBtnHtml = `<button class="btn btn-stop btn-sm stop-monitoring-btn"
        data-session-id="${esc(ev.id)}" data-site-name="${esc(ev.siteName || 'Terminal')}"
        data-testid="stop-monitoring-${esc(ev.id)}" title="Force-stop the PTY session if it's stuck">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg>Stop
      </button>`;
    }

    let siteIcon = '';
    if (ev.site === 'cli') {
      siteIcon = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px;vertical-align:middle;position:relative;top:-1px;"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>`;
    } else {
      const domain = SITES.find(s => s.id === ev.site)?.domain;
      if (domain) siteIcon = `<img src="https://www.google.com/s2/favicons?domain=${domain}&sz=16" width="14" height="14" style="margin-right:6px;vertical-align:middle;border-radius:2px;" onerror="this.style.display='none'">`;
    }

    // ── FIX: proper <div class="card-actions"> and real ${} interpolation ──
    html += `
      <div class="event-card ${esc(ev.category || ev.eventType)}"
        ...
        <div class="event-meta">
          ${showCategory ? `<span class="badge badge-${esc(ev.category)}" data-testid="event-category">${prettyCategory(ev.category)}</span>` : ''}
          <span class="badge badge-${esc(ev.eventType)}" data-testid="event-badge">${esc(ev.eventType)}</span>
        ...
        ${ev.messageSnippet ? `<div class="event-snippet" data-testid="event-snippet-${esc(ev.id)}">${esc(ev.messageSnippet)}</div>` : ''}
        <div class="card-actions">
      ...
      <div class="event-detail-panel" id="detail-${esc(ev.id)}" style="display:none" data-testid="event-detail-${esc(ev.id)}">
        <div class="detail-row">
          <span class="detail-key">Time</span>
          <span class="detail-val">${new Date(ev.timestamp || 0).toLocaleString()}</span>
        </div>
        ${ev.url ? `<div class="detail-row"><span class="detail-key">URL</span><span class="detail-val detail-url">${esc(ev.url)}</span></div>` : ''}
        ${ev.messageSnippet ? `<div class="detail-row"><span class="detail-key">Response</span><span class="detail-val detail-snippet">${esc(ev.messageSnippet)}</span></div>` : ''}
        ${ev.classificationSource ? `<div class="detail-row"><span class="detail-key">Classified by</span><span class="detail-val">${esc(ev.classificationSource)}</span></div>` : ''}
        ${ev.responseLength ? `<div class="detail-row"><span class="detail-key">Size</span><span class="detail-val">${fmtBytes(ev.responseLength)}</span></div>` : ''}
      </div>`;
  }
...
async function renderSettings(el) {
  ...
  let html = `
    ...
    <div class="settings-row" data-testid="global-notifications-row">
      <div class="settings-label">
        ...
        </svg>Desktop Notifications
      </div>
      <label class="toggle">
        <input type="checkbox" class="site-toggle" data-site="__global__" data-testid="settings-global-toggle" ${!s.globalDisabled ? 'checked' : ''}>
    ...
    <div class="settings-row" data-testid="sound-row">
      <div class="settings-label">
        ...
        </svg>Notification Sound
      </div>
      <label class="toggle">
        <input type="checkbox" id="sound-toggle" data-testid="sound-toggle" ${!s.soundDisabled ? 'checked' : ''}>
        <span class="toggle-track"></span>
      </label>
    </div>
  ...
  for (const site of SITES) {
    ...
    let toggleHtml = site.readOnly
      ? `<span class="settings-readonly">Tracked via Mac App</span>`
      : `<label class="toggle"><input type="checkbox" class="site-toggle" data-site="${site.id}" data-testid="site-toggle-${site.id}" ${enabled ? 'checked' : ''}><span class="toggle-track"></span></label>`;
    let iconHtml = site.id === 'cli'
      ? `<svg class="site-favicon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="padding:2px;"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>`
      : `<img class="site-favicon" src="https://www.google.com/s2/favicons?domain=${site.domain}&sz=32" alt="${esc(site.name)}" onerror="this.style.visibility='hidden'">`;
    html += `<div class="settings-row" data-testid="site-row-${site.id}">
      <div class="settings-label">${iconHtml}<span>${esc(site.name)}</span></div>
      ${toggleHtml}
    </div>`;
  }
  ...
  document.getElementById('sound-toggle')?.addEventListener('change', async (e) => {
    ...
    await chrome.storage.local.set({ settings });
    if (!settings.soundDisabled) chrome.runtime.sendMessage({ type: 'PLAY_CHIME_PREVIEW' });
  });
  ...
  document.getElementById('test-reply-btn')?.addEventListener('click', async () => {
    await chrome.storage.local.set({
      pendingReply: {
        tabId: null, windowId: null, site: 'test', siteName: 'Test Agent',
        eventType: 'DECISION', category: 'ACTION_REQUIRED',
...
async function updateFooter() {
  ...
  if (macDot && macLabel) {
    if (macAppConnected) { macDot.classList.add('connected'); macLabel.textContent = 'Mac App'; }
    else { macDot.classList.remove('connected'); macLabel.textContent = 'Chrome only'; }
  }
}
...
async function jumpToTab(tabId, windowId, fallbackUrl) {
  ...
  try {
    ...
    if (fallbackUrl) { chrome.tabs.create({ url: fallbackUrl }); window.close?.(); }
  }
}
...
function prettyCategory(cat) {
  const map = { ACTION_REQUIRED: 'Action Required', INFORMATION: 'Information', PENDING: 'Pending', COMPLETED: 'Completed' };
