...
_ANSI_RE = re.compile(
    ...
    # Removed \r to allow correct carriage return processing
    re.VERBOSE,
)
...
class GeminiOutputBuffer:
    ...
    def get_text(self):
        # We can also filter out known Gemini status bar lines here if needed
        all_lines = self.lines + ([self.current_line] if self.current_line else [])
        cleaned_lines = [
            line for line in all_lines 
            if not line.startswith("workspace (/directory)") 
            and not line.startswith("~") 
            and not "no sandbox" in line
        ]
        return "\n".join(cleaned_lines)
...
class PTYWrapper:
    def __init__(self, cmd: list, notify_py: str, site: str, sid: str,
        ...
        self._response_buf    = GeminiOutputBuffer()
    ...
    def _maybe_fire(self, ev_type: str, force: bool, is_error: bool = False, title_override: str = None):
        snippet = self._response_buf.get_text()
        clean = snippet.strip()
